...
observed consequence at $\rho=0.958$; and under a \emph{real credential-compromise}
experiments with live RBAC, predicted confidentiality and integrity blast radius
reproduce the actually exfiltrated secrets and corrupted data, catching over-privilege
that dependency-only views ($\rho=0.69$, $0.26$) miss---while a read-only agent identity
neither reads nor tampers, and the availability result reproduces off-distribution. Because ground truth in these experiments is observed system
...
\emph{Integrity} is the dual, and we measure it the same way. We create a ConfigMap
data-of-record per service, grant real \texttt{update} RBAC on the data each service
legitimately writes---plus three over-privilege \emph{write} misconfigurations (the ad
service able to rewrite the product catalog, email the exchange rates, recommendation
the cart)---and compromise each identity by \emph{actually corrupting}
(\texttt{patch -{}-as}) every ConfigMap its RBAC permits, reading the store back to
confirm the tamper. Corruption propagates \emph{up} to every dependent that consumes
the bad data. The dependency-only prediction, which assumes each identity writes only
its own data, collapses to $\rho=0.259$ because it is blind to the write
misconfigurations that hand a low-value identity a high-fan-out store; the RBAC-aware
integrity score reproduces the measured corruption reach ($\rho=1.00$), and the
read-only agent identity corrupts \emph{zero} stores. In both channels the
...
exfiltration) and data-tampering (right, integrity: measured corruption reach) on the
live cluster, each versus a dependency-only prediction. The dependency-only view
misses the planted over-privilege misconfigurations (red labels); the RBAC-aware score
reproduces the measured reach in both channels.}
\label{fig:conf}
\end{figure*}
...
predicted confidentiality and integrity blast radius against measured exfiltration and
data-tampering; because ground truth there is observed rather than computed, they
remove the computed-oracle circularity---decisively for availability, whose oracle is
fully independent, and for confidentiality and integrity up to the shared-RBAC caveat
below.
...
evidence of that non-identity. To probe generalization beyond the app the heuristic was
tuned on, we repeat the fault injection on Bookinfo (Section~\ref{sec:offdist}), where
the same graceful-degradation pattern and confidence-aware advantage hold; still, both
live studies are small and single-cluster, and broader multi-application, multi-cluster
validation remains future work. Separately, the confidentiality and integrity
$\rho=1.00$ reflect that the estimator consumes the same real RBAC the compromise
traverses; their content is the contrast with the dependency-only $0.69$ and $0.26$,
...
injection the prediction tracks observed outages at $\rho=0.987$ (and reproduces on an
off-distribution application); on real credential-compromise and data-tampering it
reproduces the exfiltrated and corrupted sets and catches over-privilege that
...
degrees of domain admin,'' presented at DEF CON 24, 2016. [Online]. Available:
...
risky permissions,'' open-source tool, accessed 2026. [Online]. Available:
...
demo application,'' accessed 2026. [Online]. Available:
