...
{\footnotesize U.S.\ Patents Pending: App.\ Nos.\ 19/641,446 and 64/138,302}}}
...
dominates the settings we study; role-mining and rightsizing methods infer minimal
roles from assignments and observed usage~\cite{rmp,roleminer,rminesurvey,semrole},
typically treating unused permissions as removable~\cite{awsiam} and weighting them
uniformly or by usage frequency. Intrinsic permission severity has been
raised~\cite{severity} but not connected to downstream infrastructure consequence.
The usage view is orthogonal and complementary: it asks whether a permission is
needed; we ask what it endangers (Section~\ref{sec:disc}).

\textbf{Consequence assessment in cyber operations.} A parallel literature in
military cyber operations has long argued that the effect of an action must be assessed
separately from the access used to reach it: effects-based models estimate and classify
the downstream consequences of an operation---across temporal, spatial, and force
dimensions---for collateral-damage and proportionality
assessment~\cite{maathuis18,maathuis21}. That work assesses a chosen operation against a
target; we invert the setting, ranking the \emph{defender's own identities} by the
consequence their access carries. The shared premise---that access is a poor proxy for
effect---motivates the question we ask of IAM, and Section~\ref{sec:scale} returns to it
as a proportionality lens on remediation.
...
released.\footnote{Reproducibility repository: \url{https://github.com/prawalpokharel/CEI-Cloud-Identity-IEEE}.}
...
Varying fan-out depth directly makes the mechanism visible: as depth grows, the
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.93\textwidth]{figures/fig_ci_integrity.png}
...
\label{sec:offdist}
Because the dependency heuristic was developed on Online Boutique, an observed match
there is not itself generalization. We therefore repeat the availability fault
injection on Istio's Bookinfo---an application the heuristic was never tuned on, with
a different topology (a productpage aggregating a
...
topologies (Fig.~\ref{fig:remed}).

Remediation is also where the proportionality lens of effects-based
assessment~\cite{maathuis18,maathuis21} bites: a de-scoping action buys risk reduction
at the price of severed legitimate function. On the live cluster's RBAC overlay we
re-measured exfiltration reach under all 18 single-grant revocations (real role edits,
re-verified by live authorization checks and reads), with collateral taken as the
observed flow breakage of the severed edge (Section~\ref{sec:real-avail}; an upper
bound, blind to non-flow function). Three measured facts: advantage is not per-grant
separable---four of the largest grant-holder's seven revocations measure \emph{zero}
advantage, the compromise re-routing through redundant paths, including \emph{through
the planted misconfigurations}, which silently nullify legitimate revocations; the
highest-advantage revocation (1.9 sensitivity units) severs the most load-bearing edge,
breaking all five flows---an unfavorable trade-off under an effects-based
proportionality lens; and the misconfigurations occupy exactly the high-advantage,
zero-collateral frontier (1.65/0.98/0.95 at zero flows), while the seven revocations
against the largest grant-holder \emph{jointly} yield less (1.8)---showing that
privilege-count de-scoping can favor interventions whose collateral operational cost is
excessive relative to the security benefit obtained. The ranking also yields a compact defensive
target: the top fifth of workloads by blast radius account for $42$--$64\%$ of all
identity risk, far above the uniform $20\%$. Finally, the full battery scores $10^5$ identities in a little over a second
($\approx 0.01$~ms per identity), and the availability ranking is stable under every
order-preserving perturbation of Table~\ref{tab:caps}, degrading only when the
\emph{ordering} is shuffled---an ordinal, defensible judgment, not specific constants.

\begin{figure}[t]
\centering
\includegraphics[width=0.92\columnwidth]{figures/fig_e10_remediation.png}
...
dependency-only views miss. As identity sprawl outpaces human review, prioritizing by
propagated consequence rather than access is both necessary and, atop dependency
analysis infrastructure already performs, practical.
...
pp.~1--55, 2025.
\bibitem{maathuis18} C.~Maathuis, W.~Pieters, and J.~van~den~Berg, ``Assessment
methodology for collateral damage and military (dis)advantage in cyber operations,'' in
\emph{Proc. IEEE MILCOM}, 2018, pp.~438--443.
\bibitem{maathuis21} C.~Maathuis, W.~Pieters, and J.~van~den~Berg, ``Decision support
model for effects estimation and proportionality assessment for targeting in cyber
operations,'' \emph{Defence Technology}, vol.~17, no.~2, pp.~352--374, 2021.
\bibitem{ob} Google Cloud Platform, ``Online Boutique'' microservices demo, accessed 2026.
\end{thebibliography}
